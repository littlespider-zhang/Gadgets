...
def _czi_extract_frames(arr: np.ndarray) -> list:
    ...
    # 特殊：3D PIL 风格 (H, W, C)，最后轴很小且远小于前两轴
    if ndim == 3:
        s0, s1, s2 = arr.shape
        if s2 <= 4 and s2 < s0 and s2 < s1:
            return [_normalize_to_uint8(arr.max(axis=2))]

    # 通用路径：最后两轴是 (H, W)，前面的轴按大小分为通道轴和帧轴
    prefix = arr.shape[:-2]
    channel_axes = [i for i, s in enumerate(prefix) if s <= 4]

    # 从高到低对通道轴做最大投影（轴下标在每次操作后会移动）
    reduced = arr
    ...
    # 展平所有剩余前缀轴为帧
    n_frames = 1
    for s in reduced.shape[:-2]:
        n_frames *= s
    flat = reduced.reshape(n_frames, reduced.shape[-2], reduced.shape[-1])
    return [_normalize_to_uint8(flat[i]) for i in range(n_frames)]
...
def normalize_intensities(results: list, baseline_frames: int = 2) -> float:
    """
    功能1：对 FRAP_intensity 做归一化。
    以前 baseline_frames 帧（默认 2 帧）的 FRAP_intensity 均值作为 100%，
    对所有帧计算 FRAP_intensity_norm（百分比），原地写入每个 row。

    返回所用的 baseline 值（前几帧均值）。若无有效基线则返回 0。
    """
    if not results:
        return 0.0
    n = min(baseline_frames, len(results))
    base_vals = [r["FRAP_intensity"] for r in results[:n]]
    baseline = sum(base_vals) / len(base_vals) if base_vals else 0.0
    for r in results:
        if baseline > 0:
            r["FRAP_intensity_norm"] = r["FRAP_intensity"] / baseline * 100.0
        else:
...
def load_frap_results(csv_path: str) -> dict:
    ...
    FRAP_intensity 均值为 100% 重新计算归一化。
    """
...
class AutoFRAPApp(tk.Tk):
    ...
    def _run_batch(self):
        ...
        # ── 功能1：归一化（以前两帧 FRAP_intensity 均值为 100%）──
        baseline = normalize_intensities(results, baseline_frames=2)
        if baseline > 0:
            self.log(f"  归一化基线（前两帧均值）= {baseline:.4f} → 100%")
        else:
            self.log("  归一化：基线无效（帧数不足或强度为 0），归一化列全部置 0。")
        ...
        fields = ["frame_index", "filename",
                  ...
                  "FRAP_intensity", "FRAP_intensity_norm", "pixel_count"]
        ...
        messagebox.showinfo(
            ...
            f"CSV 结果（含归一化列 FRAP_intensity_norm）:\n{out_path}\n\n"
            f"标注图片:\n{marked_dir}/\n\n"
            "可点击「◫ 结果图表」查看曲线。"
        )
