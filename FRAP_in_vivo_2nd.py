"""
FRAP_in_vivo.py — 活体 FRAP 分析工具（含动态液滴追踪）
From v4 (by zyy)
================================================================

功能概述
--------
对一组按时间排序的荧光显微图像（jpg/png/tif/czi）做 FRAP（荧光漂白后
恢复）定量分析：自动/手动定位漂白圆、逐帧追踪液滴质心、计算圆内强度，
并完成背景扣除、参考区光漂白校正与归一化，最终导出 CSV 与标注图，可在
内置面板中绘制恢复曲线与液滴轨迹。

代码架构（按数据流自底向上分节）
--------------------------------
  §1  图像 I/O           —— 把任意支持格式读成统一的 RGB / 灰度数组
  §2  CZI 解压           —— 把 CZI stack 拆成逐帧 JPEG
  §3  几何与强度基元      —— ROI 裁切、质心、偏移、亚像素加权圆内强度
  §4  自动圆检测          —— Otsu + 连通域，自动定位 FRAP 淬灭圆
  §5  校正与归一化        —— 背景扣除、参考光漂白校正、单/双点归一化
  §6  帧标注              —— 在原图上叠加 ROI/质心/各区圆环
  §7  批处理驱动          —— 串起上述步骤，逐帧产出结果行
  §8  结果 CSV I/O        —— 读回 FRAP_results.csv 供绘图
  §9  GUI 主窗口          —— AutoFRAPApp：参数、按钮、日志与流程调度
  §10 GUI 交互窗口        —— ROI/圆心选择、自动检测、参考/背景选点、预览
  §11 GUI 结果面板        —— ResultPanelWindow：matplotlib 曲线与轨迹图

可选依赖
--------
  czifile     读取 .czi（缺失则禁用 CZI 相关功能）
  tifffile    可靠读取多页/16-bit TIFF（缺失时回退 Pillow）
  matplotlib  结果图表面板（缺失则禁用「结果图表」）
"""
...
# ────────────────────────────────────────────────────────────
#  §1  Image I/O — 多格式读取
#  把 jpg/png/tif/czi 统一读成 RGB 或灰度 numpy 数组
# ────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────
#  §2  CZI Stack → JPEG 解压
#  把单个 CZI stack 拆分为逐帧灰度 JPEG（全栈统一拉伸）
# ────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────
#  §3  几何与强度基元
#  ROI 裁切、质心、偏移、亚像素加权圆内强度——像素级计算的核心
# ────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────
#  §4  FRAP 圆自动检测
#  Otsu 阈值 + 8-连通域，在框选范围内自动定位淬灭圆的圆心与半径
# ────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────
#  §5  校正与归一化
#  背景扣除、参考区光漂白校正，及单点/双点归一化（数据流：先校正后归一化）
# ────────────────────────────────────────────────────────────

def apply_reference_correction(results: list,
                               baseline_frames: int = 2) -> dict:
    """
    背景扣除 + Reference 光漂白校正，生成两列原始（未归一化）信号：

      · FRAP_intensity_subBg
            = FRAP_intensity - background_intensity            （仅需 Background 区）

      · FRAP_intensity_subBg_withRef
            = FRAP_intensity_subBg * (ref_baseline_subBg / ref_subBg(当前帧))
        其中  ref_subBg          = reference_intensity - background_intensity
              ref_baseline_subBg = baseline 阶段（前 baseline_frames 帧）ref_subBg 的均值
        （需同时有 Background 区与 Reference 区）

    原理：成像激光使整个视野缓慢漂白，Reference 区记录了这一衰减趋势；默认
    Reference 区真实荧光恒定，故用 baseline / 当前帧 的比值缩放 FRAP，抵消漂白。

    结果原地写入每个 row。缺少对应区域时跳过对应列：
      · 无 Background          → 不产出 subBg / subBg_withRef
      · 有 Background 无 Reference → 仅产出 subBg

    返回 dict：
      {"has_subBg": bool, "has_withRef": bool, "ref_baseline": float}
    """
    out = {"has_subBg": False, "has_withRef": False, "ref_baseline": 0.0}
    if not results:
        return out

    n = max(1, min(int(baseline_frames), len(results)))
    has_bg = all("background_intensity" in r for r in results)
    has_ref = all("reference_intensity" in r for r in results)

    # ── ① 背景扣除：FRAP_intensity_subBg ─────────────────────
    if has_bg:
        for r in results:
            r["FRAP_intensity_subBg"] = (r.get("FRAP_intensity", 0.0)
                                         - r.get("background_intensity", 0.0))
        out["has_subBg"] = True

    # ── ② Reference 光漂白校正：FRAP_intensity_subBg_withRef ──
    if has_bg and has_ref:
        ref_subBg = [r.get("reference_intensity", 0.0)
                     - r.get("background_intensity", 0.0) for r in results]
        ref_baseline = sum(ref_subBg[:n]) / n if n else 0.0
        for r, rs in zip(results, ref_subBg):
            scale = (ref_baseline / rs) if rs != 0 else 0.0
            r["FRAP_intensity_subBg_withRef"] = r["FRAP_intensity_subBg"] * scale
        out["ref_baseline"] = ref_baseline
        out["has_withRef"] = True
...
def normalize_intensities(results: list,
                          ...
                          source_key: str = "FRAP_intensity",
                          out_key: str = "FRAP_intensity_norm",
                          single_point: bool = False,
                          clamp: bool = True) -> dict:
    """
    FRAP 归一化，支持两种模式。

    双点归一化（single_point=False，默认）：
        以前 baseline_frames 帧 source_key 列的均值作为 100%（pre-bleach baseline），
        以 bleach_frame（漂白后第一帧）的 source_key 值作为 0%：

            norm = (I - I_bleach) / (baseline - I_bleach) * 100

        baseline → 100%，bleach 帧 → 0%，恢复曲线自 0% 回升。
        clamp=True 时保留 >100% 的过冲（恢复可能超过 baseline，属有效信息），
        仅把低于 bleach 帧的 <0% 噪声裁剪到 0。

    单点归一化（single_point=True）：
        无论 bleach_frame 取何值，一律退化为

            norm = I / baseline * 100

        即只把 baseline 帧均值记为 100%，不设 0 点（不裁剪上限）。

    source_key / out_key：被归一化的源列名与输出列名。

    参数
    ----
    results         : batch_analysis 返回的逐帧字典列表
    baseline_frames : 前多少帧的均值作为 baseline（100%），默认 2
    bleach_frame    : 漂白后第一帧的帧号（1-based）。仅双点模式使用；
                      单点模式忽略此参数。
    single_point    : True → 单点归一化（I/baseline*100，忽略 bleach_frame）
    clamp           : True → 双点结果保留 >100% 过冲，仅裁剪 <0%（单点模式不裁剪）

    返回
    ----
    dict 包含 baseline, i_bleach（单点模式 i_bleach=0）。
    """
    ...
    # ── 主 FRAP 通道（归一化 source_key → out_key）──────────
    base_vals = [r.get(source_key, 0.0) for r in results[:n]]
    baseline = sum(base_vals) / len(base_vals) if base_vals else 0.0

    if single_point:
        # 单点：忽略 bleach_frame，i_bleach 恒为 0 → norm = I / baseline * 100
        i_bleach = 0.0
    elif bleach_frame is not None and bleach_frame >= 1:
        idx = min(int(bleach_frame) - 1, len(results) - 1)
        i_bleach = results[idx].get(source_key, 0.0)
    else:
        i_bleach = 0.0
    ...
    for r in results:
        if denom != 0:
            val = (r.get(source_key, 0.0) - i_bleach) / denom * 100.0
            # 双点归一化：保留 >100% 的过冲，仅把低于 bleach 帧（<0%）的
            # 噪声裁剪到 0（单点模式不裁剪）。
            if clamp and not single_point:
                val = max(0.0, val)
            r[out_key] = val
        else:
            r[out_key] = 0.0
...
# ────────────────────────────────────────────────────────────
#  §6  帧标注（绘制叠加层）
#  在原始彩色帧上画 ROI 框、质心十字、FRAP/参考/背景圆环
# ────────────────────────────────────────────────────────────
...
def annotate_frame(filepath: str,
                   ...
                   ref_global: tuple = None,
                   bg_global: tuple = None) -> Image.Image:
    """
    在原始彩色图片上绘制标注并返回 PIL Image：
      · ROI            : 黄色虚线矩形
      · Droplet 质心   : 黄色十字（ROI 坐标 → 全图坐标）
      · FRAP 圆        : 蓝色实线圆 + 圆心点
      · Reference 圆   : 绿色实线圆（若提供 ref_global，半径同 FRAP）
      · Background 圆  : 品红实线圆（若提供 bg_global，半径同 FRAP）

    ref_global / bg_global：Reference / Background 区圆心全图坐标 (x, y)。
        二者半径与 FRAP 圆相同（frap_radius）。未提供时跳过对应绘制。
    """
    ...
    # Reference 圆：绿色实线（半径同 FRAP）
    if ref_global is not None:
        rfx, rfy = ref_global
        draw.ellipse([(rfx - r, rfy - r), (rfx + r, rfy + r)],
                     outline="#2ecc71", width=2)
        draw.ellipse([(rfx - 2, rfy - 2), (rfx + 2, rfy + 2)], fill="#2ecc71")

    # Background 圆：品红实线（半径同 FRAP）
    if bg_global is not None:
        bgx, bgy = bg_global
        draw.ellipse([(bgx - r, bgy - r), (bgx + r, bgy + r)],
                     outline="#ff6b9d", width=2)
        draw.ellipse([(bgx - 2, bgy - 2), (bgx + 2, bgy + 2)], fill="#ff6b9d")
...
def batch_analysis(folder: str,
                   ...
                   bg_center: tuple = None) -> list:
    """
    批量数据分析：对文件夹内所有支持格式（jpg/png/tif/czi）按文件名排序后逐帧计算。
    marked_dir 不为 None 时，将标注后的图片（ROI虚线框、质心、FRAP圆、
    Reference圆、Background圆）保存到该目录。
    progress_cb(i, total, filename) 可用于更新进度。
    frame_cb(row) 每帧完成后立即回调。

    ref_center : Reference 区圆心全图坐标 (x, y)。圆心由用户手动选定（固定不动），
                 半径与 FRAP 相同（frap_radius）。提供时逐帧计算 reference_intensity，
                 计算方法与 FRAP_intensity 完全一致（亚像素加权圆内均值）。
    bg_center  : Background 区圆心全图坐标 (x, y)。交互/计算与 ref_center 一致，
                 逐帧计算 background_intensity。
    ...
    for i, fp in enumerate(files):
        ...
        # Reference / Background 区强度：圆心固定（用户手选的全图坐标），
        # 半径与 FRAP 相同，计算方法与 FRAP_intensity 完全一致。
        if ref_center is not None:
            ref_i, _ = _circle_intensity(gray, ref_center[0], ref_center[1],
                                         frap_radius)
            row["reference_intensity"] = ref_i
        if bg_center is not None:
            bg_i, _ = _circle_intensity(gray, bg_center[0], bg_center[1],
                                        frap_radius)
            row["background_intensity"] = bg_i
...
# ────────────────────────────────────────────────────────────
#  §8  结果 CSV I/O
#  读回 FRAP_results.csv 的各列为 numpy 数组，供结果面板绘图
# ────────────────────────────────────────────────────────────

def load_frap_results(csv_path: str) -> dict:
    """
    读取 FRAP_results.csv，返回绘图所需各列的 numpy 数组。
    缺失或非数值的列以 None / NaN 返回（绘图端按 None 跳过该曲线）。
    """
    ...
    def opt_col(name):
        """存在且非空才读取，否则返回 None。"""
        if name in rows[0] and rows[0].get(name, "") != "":
            return col(name)
        return None

    return {
        "frame_index": col("frame_index", lambda x: int(float(x))),
        "droplet_center_x": col("droplet_center_x"),
        "droplet_center_y": col("droplet_center_y"),
        # 单点归一化（仅 baseline→100%，无 0 点）
        "FRAP_intensity_subBg_single": opt_col("FRAP_intensity_subBg_single"),
        "FRAP_intensity_subBg_withRef_single":
            opt_col("FRAP_intensity_subBg_withRef_single"),
        # 双点归一化（baseline→100%，bleach→0%）
        "FRAP_intensity_subBg_norm": opt_col("FRAP_intensity_subBg_norm"),
        "FRAP_intensity_subBg_withRef_norm":
            opt_col("FRAP_intensity_subBg_withRef_norm"),
    }


# ────────────────────────────────────────────────────────────
#  §9  GUI — 主应用窗口
#  AutoFRAPApp：参数输入、按钮调度、参数显示与日志，串起整个流程
# ────────────────────────────────────────────────────────────
...
class AutoFRAPApp(tk.Tk):
    ...
    持有整套分析参数（ROI 角点、FRAP 圆心/偏移、FRAP 半径、参考/背景区
    圆心、归一化用的 baseline 帧数与 bleach 帧号），提供文件夹选择、各功能
    按钮、参数实时显示区与日志区，并负责调度各交互窗口与批处理/绘图流程。
    所有子窗口都通过持有的 app 引用读写这里的状态。
    """
    ...
    def __init__(self):
        """构造主窗口：初始化全部内部状态（ROI 角点、FRAP 圆心、偏移、参考/背景圆心、归一化参数）并搭建 UI。"""
        super().__init__()
        ...
        # Reference / Background region centres (full-image pixel coords)
        # 圆心由用户手动选定，半径与 FRAP region 相同。
        self.reference_center_x = None
        self.reference_center_y = None
        self.background_center_x = None
        self.background_center_y = None
    ...
    def _build_ui(self):
        """搭建主界面：标题、文件夹与参数输入行、功能按钮行、参数显示区与日志区。"""
        PAD = 10
        BG = "#1a1a2e"
        CARD = "#16213e"
        ACCENT = "#0f3460"
        ...
        tk.Button(btn_frame, text="◍  Ref. 选择",
                  bg="#1a5c3a", fg="white", activebackground="#237a50",
                  command=self._run_pick_reference, **btn_style).pack(side="left", padx=4)
        tk.Button(btn_frame, text="◍  Bg. 选择",
                  bg="#7a3a5c", fg="white", activebackground="#a04e7a",
                  command=self._run_pick_background, **btn_style).pack(side="left", padx=4)
        ...
        params = [
            ...
            ("offset_x", "offset_y", "", ""),
            ("Reference_center_x", "Reference_center_y",
             "Background_center_x", "Background_center_y"),
        ]
        self._param_vars = {}
    ...
    def _set_param(self, name: str, value):
        """把单个参数值写入参数显示区（None→占位符，float→保留两位小数）。"""
        if value is None:
    ...
    def _refresh_params(self):
        ...
        mapping = {
            ...
            "Reference_center_y": self.reference_center_y,
            "Background_center_x": self.background_center_x,
            "Background_center_y": self.background_center_y,
        }
        for k, v in mapping.items():
            self._set_param(k, v)

    def _browse_folder(self):
        """弹出目录选择对话框，记录所选数据文件夹路径。"""
    ...
    def _validate_roi(self) -> bool:
        """校验 ROI 四角是否已设置；缺失则弹窗提示并返回 False。"""
    ...
    def _validate_all(self) -> bool:
        """在 ROI 校验基础上，进一步校验 FRAP 圆心与偏移量是否就绪。"""
    ...
    def _run_offset_calc(self):
        """功能①（手动）：选参考帧并打开 OffsetWindow，进入 ROI→圆心选择流程。"""
    ...
    # ── Feature 1c: Pick Reference / Background region centre ───

    def _run_pick_reference(self):
        """
        Reference 选择：打开一张（带 ROI/FRAP 标注的）图片，单击一个坐标点，
        记录为 Reference_center_x / Reference_center_y（全图坐标）。
        圆半径与 FRAP region 相同。
        """
        fp = self._pick_frame("选择一帧用于选定 Reference 区圆心")
        if not fp:
            return
        self.log(f"Reference 选择 — 图片：{os.path.basename(fp)}")
        PointPickerWindow(self, fp, mode="reference")

    def _run_pick_background(self):
        """
        Background 选择：交互逻辑与 Reference 选择完全一致，
        记录为 Background_center_x / Background_center_y（全图坐标）。
        圆半径与 FRAP region 相同。
        """
        fp = self._pick_frame("选择一帧用于选定 Background 区圆心")
        if not fp:
            return
        self.log(f"Background 选择 — 图片：{os.path.basename(fp)}")
        PointPickerWindow(self, fp, mode="background")

    # ── Feature 2: Image Preview ─────────────────────────────────

    def _run_preview(self):
        """功能②：参数就绪后选一帧打开 PreviewWindow，预览 ROI 与 FRAP 圆叠加效果。"""
    ...
    def _run_batch(self):
        """功能③：批处理整个文件夹——逐帧计算、背景/参考校正、单双点归一化，导出 CSV 与标注图。"""
        if not self._validate_all():
            return
        folder = self.folder_path.get()
        if not folder:
        ...
        # Reference / Background 区圆心（全图坐标）。两者都选定才进行
        # FRAP_intensity_subBg_withRef 校正；缺一则仅跳过相应列，不影响其余分析。
        ref_center = None
        if None not in (self.reference_center_x, self.reference_center_y):
            ref_center = (self.reference_center_x, self.reference_center_y)
        bg_center = None
        if None not in (self.background_center_x, self.background_center_y):
            bg_center = (self.background_center_x, self.background_center_y)

        if ref_center is not None:
            self.log(f"  Reference 区圆心（全图）= {ref_center}，半径={radius}px")
        if bg_center is not None:
            self.log(f"  Background 区圆心（全图）= {bg_center}，半径={radius}px")
        if ref_center is None or bg_center is None:
            self.log("  ⚠ 未同时选定 Reference 与 Background 区，"
                     "本次不产出 FRAP_intensity_subBg_withRef 校正列。")
        ...
        # ── 第一步：背景扣除 + Reference 光漂白校正（原始信号）──
        #   FRAP_intensity_subBg          = FRAP_intensity - background_intensity
        #   FRAP_intensity_subBg_withRef  = FRAP_intensity_subBg * (ref_baseline / ref_当前帧)
        corr = apply_reference_correction(results, baseline_frames=baseline_frames)
        if corr["has_subBg"]:
            self.log("  ① 背景扣除 → FRAP_intensity_subBg = FRAP - background")
        if corr["has_withRef"]:
            self.log(f"  ① Reference 校正 → FRAP_intensity_subBg_withRef"
                     f"（背景扣除后参考 baseline={corr['ref_baseline']:.4f}，"
                     f"缩放因子 = baseline / 当前帧）")
        if not corr["has_subBg"]:
            self.log("  ⚠ 未选定 Background 区，本次不产出 subBg / subBg_withRef 及其归一化列。")
        elif not corr["has_withRef"]:
            self.log("  ⚠ 未选定 Reference 区，本次不产出 subBg_withRef 及其归一化列。")

        # ── 第二步：双点归一化（baseline→100%，bleach 帧→0%）──
        #   以及 单点归一化（仅 baseline→100%，不用 bleach 帧作 0 点）。
        if corr["has_subBg"]:
            no = normalize_intensities(
                results, baseline_frames=baseline_frames, bleach_frame=bleach_frame,
                source_key="FRAP_intensity_subBg",
                out_key="FRAP_intensity_subBg_norm")
            self.log(f"  ② 双点归一化(subBg)：baseline={no['baseline']:.4f}→100%，"
                     f"bleach帧#{bleach_frame}={no['i_bleach']:.4f}→0%")
            # 单点归一化：single_point=True → 恒为 I/baseline*100（忽略 bleach 帧）
            ns = normalize_intensities(
                results, baseline_frames=baseline_frames,
                source_key="FRAP_intensity_subBg",
                out_key="FRAP_intensity_subBg_single",
                single_point=True)
            self.log(f"  ② 单点归一化(subBg)：baseline={ns['baseline']:.4f}→100%（无 0 点）")
        if corr["has_withRef"]:
            no = normalize_intensities(
                results, baseline_frames=baseline_frames, bleach_frame=bleach_frame,
                source_key="FRAP_intensity_subBg_withRef",
                out_key="FRAP_intensity_subBg_withRef_norm")
            self.log(f"  ② 双点归一化(subBg_withRef)：baseline={no['baseline']:.4f}→100%，"
                     f"bleach帧#{bleach_frame}={no['i_bleach']:.4f}→0%")
            ns = normalize_intensities(
                results, baseline_frames=baseline_frames,
                source_key="FRAP_intensity_subBg_withRef",
                out_key="FRAP_intensity_subBg_withRef_single",
                single_point=True)
            self.log(f"  ② 单点归一化(subBg_withRef)：baseline={ns['baseline']:.4f}→100%（无 0 点）")
        ...
        fields = ["frame_index", "filename",
                  ...
                  "reference_intensity",
                  "FRAP_intensity_subBg",
                  "FRAP_intensity_subBg_single",
                  "FRAP_intensity_subBg_norm",
                  "FRAP_intensity_subBg_withRef",
                  "FRAP_intensity_subBg_withRef_single",
                  "FRAP_intensity_subBg_withRef_norm",
                  "pixel_count"]
...
# ────────────────────────────────────────────────────────────
#  §10  GUI — 交互工作流窗口
#  ROI/圆心选择、自动检测、参考/背景选点、预览（构造签名依赖主窗口已定义）
# ────────────────────────────────────────────────────────────
...
class OffsetWindow(tk.Toplevel):
    ...
    def __init__(self, app: AutoFRAPApp, filepath: str, next_step=None):
        """载入参考帧并按上限缩放显示，准备 ROI 两次点击状态；next_step 决定 Continue 后进入的窗口类。"""
        super().__init__(app)
        self.app = app
        ...
        self._click_pts = []  # 2 ROI canvas pts
        self._rect_id = None

        self._build()

    def _expected_clicks(self) -> int:
        """本窗口期望的点击次数（ROI 需两次：左上角、右下角）。"""
        return 2

    def _build(self):
        """搭建窗口：提示、整帧画布、状态标签与 Reset/Continue 按钮。"""
        BG = "#1a1a2e"
        CARD = "#16213e"
        ACCENT = "#0f3460"
        HIGHLIGHT = "#e94560"
    ...
    def _on_click(self, event):
        """记录一次点击并换算回原图坐标：第 1 点=ROI 左上角，第 2 点=右下角并画出矩形。"""
    ...
    def _continue(self):
        """规整角点使 corner1 恒为左上角，校验通过后打开下一步窗口（默认手动选点 / 自动检测）。"""
...
class FRAPPickerWindow(tk.Toplevel):
    ...
    def __init__(self, app: AutoFRAPApp, filepath: str, parent_win: OffsetWindow):
        """裁出 ROI 并 4× 放大显示，计算参考帧质心，准备手动点选 FRAP 圆心。"""
        super().__init__(app)
        self.app = app
    ...
    def _build(self):
        """搭建窗口：放大 ROI 画布、质心十字标记、状态标签与 Reset/Finish 按钮。"""
        BG = "#1a1a2e"
        CARD = "#16213e"
        ACCENT = "#0f3460"
        TEXT = "#eaeaea"
    ...
    def _on_click(self, event):
        ...
        """记录 FRAP 圆心（ROI 坐标），据质心算出偏移量并画出蓝色 FRAP 圆。"""
    ...
    def _finish(self):
        """校验已选圆心后关闭本窗口与上游 OffsetWindow，结束偏移量计算流程。"""
...
class AutoDetectWindow(tk.Toplevel):
    ...
    def __init__(self, app: AutoFRAPApp, filepath: str, parent_win):
        """裁出 ROI 并 4× 放大，计算质心，准备两次点击框选 FRAP 大致范围以触发自动检测。"""
        super().__init__(app)
        self.app = app
        self.parent_win = parent_win
    ...
    # ── UI ──────────────────────────────────────────────────
    def _build(self):
        """搭建窗口：放大 ROI 画布、状态标签与 重新框选/重新检测/Finish 按钮。"""
    ...
    def _run_detection(self):
        """在框选范围内调用 detect_frap_circle 求圆心/半径，写回主程序参数并绘制结果。"""
        if len(self._box_pts) < 2:
...
class PointPickerWindow(tk.Toplevel):
    """
    Reference / Background 区圆心选择窗口。

    展示一张（若 ROI/FRAP 参数已就绪则带标注的）整帧图片，用户单击一个点，
    该点记为：
      · mode="reference"  → app.reference_center_x / reference_center_y
      · mode="background" → app.background_center_x / background_center_y
    坐标为全图像素坐标，半径与 FRAP region 相同（仅用于显示圆环）。
    两个模式交互逻辑完全一致；坐标实时写入主界面参数显示区。
    """
    _CANVAS_MAX = 700

    def __init__(self, app: AutoFRAPApp, filepath: str, mode: str = "reference"):
        """Reference/Background 圆心选择窗口：按模式设色，载入（可能带标注的）底图并缩放显示。"""
        super().__init__(app)
        self.app = app
        self.filepath = filepath
        self.mode = mode  # "reference" | "background"
        ...
        if mode == "reference":
            ...
            self._color = "#ff6b9d"
            self._label = "Background"

        self.title(f"选择 {self._label} 区圆心（全图坐标）")
        self.configure(bg="#1a1a2e")
        self.resizable(False, False)

        # 构建（可能带标注的）底图
        self.pil_img = self._make_base_image(filepath)
        self.orig_w, self.orig_h = self.pil_img.size
        self.scale = min(self._CANVAS_MAX / self.orig_w,
                         self._CANVAS_MAX / self.orig_h, 1.0)
        dw = int(self.orig_w * self.scale)
    ...
    def _make_base_image(self, filepath: str) -> Image.Image:
        """生成底图：若 ROI/FRAP 参数就绪则叠加 ROI+FRAP 标注，并叠加另一区已选圆。"""
        app = self.app
        # 另一区的已选圆心（让用户能同时看到两区，避免重叠）
        ref_g = None
        if (self.mode != "reference"
                and None not in (app.reference_center_x, app.reference_center_y)):
            ref_g = (app.reference_center_x, app.reference_center_y)
        bg_g = None
        if (self.mode != "background"
                and None not in (app.background_center_x, app.background_center_y)):
            bg_g = (app.background_center_x, app.background_center_y)

        roi_ready = None not in (app.corner1_x, app.corner1_y,
                                 app.corner2_x, app.corner2_y,
                                 app.offset_x, app.offset_y)
        if roi_ready:
            try:
                gray = load_image_as_gray(filepath)
                res = calc_frap_intensity(
                    gray, app.corner1_x, app.corner1_y,
                    app.corner2_x, app.corner2_y,
                    app.offset_x, app.offset_y, app.frap_radius.get())
                return annotate_frame(
                    filepath,
                    app.corner1_x, app.corner1_y,
                    app.corner2_x, app.corner2_y,
                    res["frap_x_global"], res["frap_y_global"],
                    app.frap_radius.get(),
                    res["droplet_center_x"], res["droplet_center_y"],
                    ref_global=ref_g, bg_global=bg_g)
            except Exception:
                pass  # 标注失败则退回原图

        # 退回：原图（仍叠加另一区已选圆，便于参照）
        img = open_as_rgb(filepath)
        if ref_g or bg_g:
            draw = ImageDraw.Draw(img)
            r = app.frap_radius.get()
            if ref_g:
                draw.ellipse([(ref_g[0] - r, ref_g[1] - r),
                              (ref_g[0] + r, ref_g[1] + r)],
                             outline="#2ecc71", width=2)
            if bg_g:
                draw.ellipse([(bg_g[0] - r, bg_g[1] - r),
                              (bg_g[0] + r, bg_g[1] + r)],
                             outline="#ff6b9d", width=2)
        return img

    def _build(self):
        """搭建窗口：提示、底图画布（若已有选点则预绘）、状态标签与 Reset/Finish 按钮。"""
        BG = "#1a1a2e"
        CARD = "#16213e"
        ACCENT = "#0f3460"
        TEXT = "#eaeaea"
        ...
        hint = (f"左键单击选定 {self._label} 区圆心"
                f"（黄=ROI，蓝=FRAP，绿=Ref，粉=Bg；圆半径 = FRAP 半径）")
        tk.Label(self, text=hint,
                 font=("Courier New", 9), fg=MUTED, bg=BG).pack(pady=(8, 2))
        ...
        # 若当前模式已有选点，先画出来
        cur = self._current_center()
        if None not in cur:
            self._draw_marker(cur[0] * self.scale, cur[1] * self.scale)

        self.status_var = tk.StringVar(
            value=(f"{self._label} 圆心（全图）：{cur}"
                   if None not in cur else f"请单击选定 {self._label} 区圆心"))
        tk.Label(self, textvariable=self.status_var,
                 font=("Courier New", 9, "bold"),
                 fg=self._color, bg=BG).pack(pady=4)
    ...
    def _current_center(self):
        """返回当前模式（reference/background）已记录的圆心坐标元组。"""
        if self.mode == "reference":
            return (self.app.reference_center_x, self.app.reference_center_y)
        return (self.app.background_center_x, self.app.background_center_y)

    def _draw_marker(self, cx_canvas, cy_canvas):
        """在画布指定位置绘制当前区的圆环与圆心点（半径同 FRAP）。"""
        for it in self._marker_items:
            self.canvas.delete(it)
        self._marker_items = []
        r = self.app.frap_radius.get() * self.scale
        self._marker_items.append(self.canvas.create_oval(
            cx_canvas - r, cy_canvas - r, cx_canvas + r, cy_canvas + r,
            outline=self._color, width=2))
        self._marker_items.append(self.canvas.create_oval(
            cx_canvas - 3, cy_canvas - 3, cx_canvas + 3, cy_canvas + 3,
            fill=self._color, outline=""))

    def _on_click(self, event):
        """记录点击点为当前区圆心（全图坐标），刷新参数并重绘标记。"""
        ox = int(event.x / self.scale)
        oy = int(event.y / self.scale)
        if self.mode == "reference":
            self.app.reference_center_x = ox
            self.app.reference_center_y = oy
        else:
            self.app.background_center_x = ox
            self.app.background_center_y = oy
        self.app._refresh_params()
        self._draw_marker(event.x, event.y)
        self.status_var.set(f"{self._label} 圆心（全图）：({ox}, {oy})")
        self.app.log(f"  {self._label} 圆心选定：全图 ({ox}, {oy})")

    def _reset(self):
        """清除当前区已选圆心与画布标记。"""
        if self.mode == "reference":
            self.app.reference_center_x = self.app.reference_center_y = None
        else:
            self.app.background_center_x = self.app.background_center_y = None
        self.app._refresh_params()
        for it in self._marker_items:
            self.canvas.delete(it)
        self._marker_items = []
        self.status_var.set(f"请单击选定 {self._label} 区圆心")
        self.app.log(f"  {self._label} 圆心已重置。")

    def _finish(self):
        """校验已选圆心后关闭窗口。"""
        if None in self._current_center():
            messagebox.showwarning("未选定", f"请先单击选定 {self._label} 区圆心。")
            return
        self.app.log(f"  {self._label} 选择完成，窗口关闭。")
        self.destroy()
...
class PreviewWindow(tk.Toplevel):
    ...
    def __init__(self, app: AutoFRAPApp, filepath: str):
        """载入整帧，按当前参数算出本帧 FRAP 位置与质心，构建界面并先显示整帧视图。"""
        super().__init__(app)
        self.app = app
        self.filepath = filepath
        self.title("图像预览")
    ...
    def _annotated_roi(self) -> Image.Image:
        """返回 4× 放大的 ROI 视图，叠加 FRAP 圆与质心标记。"""
        cx1, cy1 = self.app.corner1_x, self.app.corner1_y
        cx2, cy2 = self.app.corner2_x, self.app.corner2_y
    ...
    def _build(self):
        """搭建窗口：提示、画布与「ROI Focus / Whole Frame」切换按钮。"""
        BG = "#1a1a2e"
        MUTED = "#8892a4"
    ...
    def _show_full(self):
        """在画布上显示整帧标注视图，并把切换按钮文案设为 ROI Focus。"""
    ...
    def _show_roi(self):
        """在画布上显示放大的 ROI 标注视图，并把切换按钮文案设为 Whole Frame。"""
...
# ────────────────────────────────────────────────────────────
#  §11  GUI — 结果图表面板
#  ResultPanelWindow：matplotlib 绘制归一化曲线、液滴坐标与移动轨迹
# ────────────────────────────────────────────────────────────

class ResultPanelWindow(tk.Toplevel):
    """
    结果图表面板：读取 FRAP_results.csv，在一个窗口中绘制（白底科研配色）：
      第一行 a) 单点归一化 (%) vs 帧编号（subBg / subBg_withRef）
            c) 双点归一化 (%) vs 帧编号（subBg / subBg_withRef）
      第二行 d) droplet_center_x vs 帧编号
            e) droplet_center_y vs 帧编号
            f) droplet 移动轨迹（center_x vs center_y，按帧着色，标出起止点）
    使用 matplotlib，嵌入 tkinter，带缩放/保存工具栏。
    """

    # 配色（白底科研配色方案）
    _BG = "#ffffff"       # figure 背景：白
    _AX_BG = "#ffffff"    # 坐标区背景：白
    _GRID = "#dddddd"     # 网格：浅灰
    _SPINE = "#000000"    # 坐标轴线：黑
    _TITLE = "#000000"    # 标题：黑
    _LABEL = "#000000"    # 轴标签/刻度：黑

    # 曲线配色：matplotlib 默认科研色板（Tableau / matplotlib C0..C3）
    _C_SUB = "#1f77b4"    # subBg：蓝
    _C_REF = "#d62728"    # subBg_withRef：红
    _C_X = "#1f77b4"      # droplet x：蓝
    _C_Y = "#ff7f0e"      # droplet y：橙
    _C_TRAJ = "#7f7f7f"   # 轨迹连线：灰
    _C_BASE = "#2ca02c"   # 100% baseline 参考线：绿
    _C_BLEACH = "#d62728" # 0% bleach 参考线：红
    _MS = 0.5             # marker size

    def __init__(self, app: AutoFRAPApp, csv_path: str):
        """读取 FRAP_results.csv，在 2×N 网格中绘制归一化曲线、液滴 X/Y 坐标与移动轨迹，并嵌入 tkinter。"""
        super().__init__(app)
        self.app = app
        ...
        subbg_single = data["FRAP_intensity_subBg_single"]            # 单点%, 可能 None
        subbg_withref_single = data["FRAP_intensity_subBg_withRef_single"]
        subbg_norm = data["FRAP_intensity_subBg_norm"]               # 双点%, 可能 None
        subbg_withref_norm = data["FRAP_intensity_subBg_withRef_norm"]
        cx = data["droplet_center_x"]
        cy = data["droplet_center_y"]
        n = len(frames)
        extras = []
        if subbg_norm is not None:
            extras.append("扣背景")
        if subbg_withref_norm is not None:
            extras.append("扣背景+参考校正")
        self.app.log(f"  读取 {n} 帧数据，开始绘图"
                     + (f"（含{'/'.join(extras)}校正曲线）。" if extras else "。"))

        fig = Figure(figsize=(13, 7.6), dpi=100, facecolor=self._BG)
        # 2 行布局：第一行 2 个 ax（单点 / 双点归一化），第二行 3 个 ax。
        gs = fig.add_gridspec(2, 6)
        ax_a = fig.add_subplot(gs[0, 0:3])   # 第一行 左：单点归一化(single)
        ax_c = fig.add_subplot(gs[0, 3:6])   # 第一行 右：双点归一化(norm)
        ax_d = fig.add_subplot(gs[1, 0:2])   # 第二行 左：droplet_x / frame
        ax_e = fig.add_subplot(gs[1, 2:4])   # 第二行 中：droplet_y / frame
        ax_f = fig.add_subplot(gs[1, 4:6])   # 第二行 右：droplet_x / droplet_y

        # 两条信号配色：subBg=蓝，subBg_withRef=红（科研基础色）
        C_SUB = self._C_SUB
        C_REF = self._C_REF

        def _plot_pair(ax, ydata_sub, ydata_ref, title, ylabel,
                       lbl_sub, lbl_ref, hline100=False):
            plotted = False
            if ydata_sub is not None:
                ax.plot(frames, ydata_sub, "-o", color=C_SUB,
                        markersize=self._MS, linewidth=1.0, label=lbl_sub)
                plotted = True
            if ydata_ref is not None:
                ax.plot(frames, ydata_ref, "-^", color=C_REF,
                        markersize=self._MS, linewidth=1.0, label=lbl_ref)
                plotted = True
            if hline100:
                ax.axhline(100, color=self._C_BASE, linestyle="--",
                           linewidth=0.9, label="100% (baseline)")
            self._style_ax(ax, title, "Frame Index", ylabel)
            if plotted:
                leg = ax.legend(loc="best", fontsize=7, framealpha=0.9)
                for t in leg.get_texts():
                    t.set_color(self._LABEL)

        def _set_norm_ylim(ax, *arrays):
            """
            归一化(%)纵轴范围：
              · 若任一曲线超过 100% → 保留过冲，自动取合适上限（max×1.05 留余量）
              · 若均未超过 100%     → 上限固定标到 120%
            下限在 0% 下方留少量余量（若有负值则跟随最小值）。
            """
            vals = []
            for a in arrays:
                if a is not None:
                    vals.extend(float(v) for v in np.asarray(a)
                                if np.isfinite(v))
            if not vals:
                return
            mx, mn = max(vals), min(vals)
            top = mx * 1.05 if mx > 100.0 else 120.0
            bottom = min(mn, 0.0) - 5.0
            ax.set_ylim(bottom, top)

        # ── a) 第一行左：单点归一化（仅 baseline→100%，无 0 点）──
        _plot_pair(ax_a, subbg_single, subbg_withref_single,
                   "Single-point Normalized (baseline=100%)",
                   "Intensity (%)", "subBg_single", "subBg_withRef_single",
                   hline100=True)
        _set_norm_ylim(ax_a, subbg_single, subbg_withref_single)

        # ── c) 第一行右：双点归一化（baseline→100%，bleach→0%）──
        # 保留 >100% 过冲；超 100% 自动定上限，否则上限标到 120%。
        _plot_pair(ax_c, subbg_norm, subbg_withref_norm,
                   "Two-point Normalized (baseline=100%, bleach=0%)",
                   "Intensity (%)", "subBg_norm", "subBg_withRef_norm",
                   hline100=True)
        ax_c.axhline(0, color=self._C_BLEACH, linestyle="--", linewidth=0.9)
        _set_norm_ylim(ax_c, subbg_norm, subbg_withref_norm)

        # ── d) 第二行左：droplet center_x vs frame ─────────────
        ax_d.plot(frames, cx, "-o", color=self._C_X,
                  markersize=self._MS, linewidth=1.0)
        self._style_ax(ax_d, "Droplet Center X",
                       "Frame Index", "center_x (px)")

        # ── e) 第二行中：droplet center_y vs frame ─────────────
        ax_e.plot(frames, cy, "-o", color=self._C_Y,
                  markersize=self._MS, linewidth=1.0)
        self._style_ax(ax_e, "Droplet Center Y",
                       "Frame Index", "center_y (px)")

        # ── f) 第二行右：droplet 轨迹 center_x vs center_y ─────
        ax_f.plot(cx, cy, "-", color=self._C_TRAJ, linewidth=0.7, alpha=0.7)
        sc = ax_f.scatter(cx, cy, c=frames, cmap="viridis", s=10, zorder=3)
        if n > 0:
            ax_f.scatter([cx[0]], [cy[0]], color=self._C_BASE, s=45,
                         marker="o", edgecolors="black", linewidths=0.5,
                         zorder=4, label="start")
            ax_f.scatter([cx[-1]], [cy[-1]], color=self._C_BLEACH, s=55,
                         marker="X", edgecolors="black", linewidths=0.5,
                         zorder=4, label="end")
        ax_f.invert_yaxis()   # 图像坐标系 y 向下，轨迹方向与图像一致
        self._style_ax(ax_f, "Droplet Movement (X vs Y)",
                       "center_x (px)", "center_y (px)")
        leg_f = ax_f.legend(loc="best", fontsize=7, framealpha=0.9)
        for t in leg_f.get_texts():
            t.set_color(self._LABEL)
        cbar = fig.colorbar(sc, ax=ax_f, fraction=0.046, pad=0.04)
    ...
    def _style_ax(self, ax, title, xlabel, ylabel):
        """对单个子图统一应用白底科研配色（标题/轴标签/刻度/网格/边框）。"""
